...
class OxfordPets(DatasetBase):
    ...
    def __init__(self, cfg, clean_data=False):
        ...
        if clean_data == False:
            self.split_path = os.path.join(self.dataset_dir, 'split_oxford_pets.json')
            train, val, test = self.read_split(self.split_path, self.image_dir)
            train = self.generate_fewshot_dataset(train, num_shots=num_shots) 
        else:        
            self.split_path = os.path.join(self.dataset_dir, 'split_cleaned_oxford_pets.json')
    ...
    def read_split(filepath, path_prefix):
        def _convert(items):
            out = []
            for impath, label, classname, incorrect_label, incorrect_classname in items:
