...
def clean_model(cfg, train_data_dict, given_dataset, re_clean=True):
    ...
    #######################
    # 表2中第1行结果复现, 记得将 cleaned_dataset = clean_model(cfg, train_data_dict, dataset, re_clean=True) 替换成 train_data_dict 替换成 test_data_dict
    ########################
    # img_feas = train_data_dict['img_feas']
    # lbs = train_data_dict['lbs']
    # gt_lbs = train_data_dict['gt_lbs']
    ...
    # logits_yes = train_data_dict['logits_yes'].to(torch.float32)
    # probs_no = train_data_dict['probs_no']
    # # 结合 clip 和 clipn 对标签进行 zero-shot 预测
    # probs_yes = F.softmax(logits_yes, dim=1)
    # # probs = torch.matmul(probs_yes, (1-probs_no)) 
    # pred_lbs = torch.argmax(probs_yes, dim=1)  
    # acc = (pred_lbs == gt_lbs).sum() / len(gt_lbs)
    # print('0000')
    ########################
    ...
    # # probs = torch.matmul(probs_yes, (1-probs_no)) 
    # pred_lbs = torch.argmax(probs_yes, dim=1)  
    # mask_tmp = (torch.max(probs_yes, dim=1)[0] > threshold_)
    # lbs[mask_tmp] = pred_lbs[mask_tmp]
    ########################

    # ####################################################
    # # direct_and_inverse 对比 direct 方式矫正标签的正确率
    # ###################################################
    # img_feas = train_data_dict['img_feas']
    # lbs = train_data_dict['lbs']
    # gt_lbs = train_data_dict['gt_lbs']
    ...
    #         if probs_no[idx][lbs[idx]] > threshold:
    #             incorrect_indices_lst.append(idx)
    #     pred_lbs_copy = pred_lbs.clone()
    #     lbs_copy = lbs.clone()

    #     # 对有问题的样本标签进行矫正
    #     lbs_copy[incorrect_indices_lst] = pred_lbs_copy[incorrect_indices_lst]
    #     direct_inverse_corrected_num = len(lbs) - (lbs_copy == gt_lbs).sum() 
    #     print(f'threshold: {threshold:.2f}, direct_inverse_corrected_num: {direct_inverse_corrected_num:.2f}****\n')

    # # direct方法，挑选大于threshold_的值
    # for threshold_ in [0.1,0.15,0.2,0.25,0.3,0.35,0.4,0.45,0.5,0.55,0.6,0.65,0.7,0.75,0.8,0.85,0.9]:
    #     pred_lbs_copy = pred_lbs.clone()
    #     lbs_copy = lbs.clone()
    #     mask_tmp = (torch.max(probs_yes, dim=1)[0] > threshold_)
    #     lbs_copy[mask_tmp] = pred_lbs_copy[mask_tmp]
    #     direct_corrected_num = len(lbs) - (lbs_copy == gt_lbs).sum() 
    #     print(f'threshold: {threshold_:.2f}, direct_corrected_num: {direct_corrected_num:.2f}****\n')
    # #######################

    return dataset

# def remove_equal_values(file_path):
            
#     with open(file_path, 'r') as file:
#         split_cleaned_dataset = json.load(file)
    
#     # 获取需要删除的列表
#     key_to_check = "train"
#     # 创建一个集合(set)来存储已经出现过的列表
#     seen_lists = set()
...
#     # 遍历原始字典中的列表
#     for lst in split_cleaned_dataset[key_to_check]:
#         # 将列表转换为元组，因为元组是可哈希的，可以添加到集合(set)中
#         tpl = tuple(lst)
        
#         # 检查当前列表是否已经在集合(set)中出现过
#         if tpl not in seen_lists:
#             seen_lists.add(tpl)  # 将当前列表添加到集合(set)中
#             unique_lists.append(lst)  # 将当前列表添加到新的列表中

#     # 更新字典中的值为去重后的列表
#     split_cleaned_dataset[key_to_check] = unique_lists
